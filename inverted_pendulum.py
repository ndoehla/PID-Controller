    ...
    def game_round(self):
        ...
        for i in range(self.max_timestep):
            ...
            if self.PID_controller is None:
                ...
                action = self.PID_controller.get_action(self.pendulum.get_state(), self.surface_array, random_controller=self.random_controller)
                for event in pygame.event.get():
                    if event.type == QUIT:
                        pygame.quit()
                        sys.exit()
            ...
            time_step_list.append(timestep)
...
def get_args():
    ...
    parser.add_argument('--random_controller', type=bool, default=False) #original default was true, change to false for PID controller
